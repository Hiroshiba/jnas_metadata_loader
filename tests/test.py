...
class Test(unittest.TestCase):
    def test_all_path(self):
        paths = json.load(
            open(
                os.path.join(
                    os.path.dirname(os.path.realpath(__file__)), "wav_paths.json"
                )
            )
        )
        ...
        text_id_list = (
            ["{:03d}".format(i + 1) for i in range(150)]
            + ["P{:02d}".format(i + 1) for i in range(5)]
            + [
                "{:03d}{}".format(i, ab)
                for i in [139, 140, 141, 142, 143, 144, 145, 146]
                for ab in "AB"
            ]
        )
